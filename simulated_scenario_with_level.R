...
output_fig = "files/modeling_output/figures/margen_vs_restock_param_optimizado.png"
...
nivel = read_feather("files/datasets/model_output/nivel_restock_medio.feather")
...
simular = function(restock = 4,
                    restock_lag = 3,
                    costo_venta_remota = 3000,
                    fractor_dcto_anual = 0.1,
                    bool_venta_remota = T,
                    porcentaje_margen = 0.2){

  parametros_sim = nivel %>% 
    filter(periodo_restock == restock & costoMotoboy == bool_venta_remota) %>% 
    mutate(security_forecast = nivel_restock)
  ...
  dataset_diario = dataset_inventario %>% 
    group_by(fecha_dato) %>% 
    summarise(stock_en_traslado = as.integer(sum(stock_en_traslado)) ,
...
casos = crossing(
  restock = c(4,5,7,9,10,13,15),
  restock_lag = 3,
  costo_venta_remota = c(3000),
  fractor_dcto_anual = 0.1,
